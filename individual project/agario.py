...
def main():
	...
	def win():
		global timeScore, clock, finalTime, points, finalPoints, currentPoints
		...
		score.st()
		score.pd()
		score.write("Score: " + str(finalPoints),font = ("KacstDecorative",20,"bold") ,align="center")
		aSize = 15
		bSize = 25
		cSize = 0
		...
		if finalTime <= aSize:
			grade = "A+"
		elif finalTime > aSize and timeScore <= bSize:
			grade = "B+"
		elif finalTime > bSize and timeScore <= cSize:
			grade = "C+"
		else:
			grade = "F"
		score.pu()
	...
	#bots renweing stats vars
		...
		BAXCor = random.randint(-SCREEN_WIDTH + MAXIMUM_BALL_RADIUS, SCREEN_WIDTH - MAXIMUM_BALL_RADIUS)
		BAYCor = random.randint(-SCREEN_HEIGHT + MAXIMUM_BALL_RADIUS, SCREEN_HEIGHT - MAXIMUM_BALL_RADIUS)
		if MY_BALL.distance(BAXCor, BAYCor) <= int(MY_BALL.r*1.5*10):
			BAXCor = random.randint(-SCREEN_WIDTH + MAXIMUM_BALL_RADIUS, SCREEN_WIDTH - MAXIMUM_BALL_RADIUS)
			BAYCor = random.randint(-SCREEN_HEIGHT + MAXIMUM_BALL_RADIUS, SCREEN_HEIGHT - MAXIMUM_BALL_RADIUS)
		if MY_BALL.distance(BAXCor, BAYCor) <= int(MY_BALL.r*1.5*10):
			BAXCor = random.randint(-SCREEN_WIDTH + MAXIMUM_BALL_RADIUS, SCREEN_WIDTH - MAXIMUM_BALL_RADIUS)
		...
		for i in FOOD:
			if collide(MY_BALL,i) == True:
				...
				score.pd()
				score.write("Score: "+str(finalPoints),font = ("Purisa",30,"bold") ,align="center")
				getscreen().update()
	...
	def timerDisplay():
		...
		finalTime = timeScore - clock
		timeWrite.write("time: " + str(finalTime), False, "left", ("Purisa", 30, "bold"))
	...
	def lose():
		global clock, timeScore, RUNNING, currentPoints,points
		currentPoints = points
		clock = timeScore
		MY_BALL.ht()
		score.ht()
